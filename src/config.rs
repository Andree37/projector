...
impl TryFrom<Vec<String>> for Operation {
    ...
    fn try_from(value: Vec<String>) -> Result<Self, Self::Error> {
        ...
        if term == "rm" {
            if value.len() != 2 {
                return Err(anyhow!("operation add expects 1 argument, but got {}", value.len()- 1));
            }
        ...
        if value.len() > 1 {
            let err = anyhow!("operation prints expects 0 but got 1 or more");
            return Err(err);
        }
...
fn get_pwd(pwd: Option<PathBuf>) -> Result<PathBuf> {
    if let Some(pwd) = pwd {
     ...
     return Ok(std::env::current_dir().context("error getting current_dir")?);
}
